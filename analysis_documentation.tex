...
The complete analysis consists of five sequential steps:

\begin{enumerate}
    \item \textbf{Event Generation} (\texttt{z01\_GeneratePythiaEvents.C}): Generate Pythia8 events with jet reconstruction
    \item \textbf{Correlation Analysis} (\texttt{z02\_RunCorrelationAnalysis.sh}): Fill same-event and mixed-event correlations with proper normalization
...
Key files:
\begin{itemize}
    \item \texttt{z01\_GeneratePythiaEvents.C} - Event generation
    \item \texttt{jAnaSimple/SimpleCorrelation.C} - Correlation analysis
    \item \texttt{z03\_ExtractQuantification.C} - Quantification with fitting
    \item \texttt{z04\_PlotResults.C} - Figure generation
    \item \texttt{z05\_GenerateTables.C} - Table generation
    \item \texttt{run\_full\_workflow.sh} - Master script
\end{itemize}
