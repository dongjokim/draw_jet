...
#include<TSystem.h>
#include<TString.h>
// GUI headers are included automatically in ROOT interpreted mode
// If compiling with ACLiC, you may need to add them explicitly
#include<TVector3.h>
#include<TVector2.h>
#include<TH1D.h>
#include<TH2D.h>
...
void DrawBarrel();
void draw_jet_pythia();
void LoadPythiaEvent();
void DrawArrow();
void make_gui();
void FillDeltaPlots();
void DrawDeltaPlots();
...
void draw_jet_pythia(){
    // Note: Libraries should be loaded first using setup_pythia.C
    // If not loaded, try loading them here as fallback
    if(gSystem->Load("libpythia8") < 0) {
        TString pythia8Path = gSystem->Getenv("PYTHIA8");
        if(pythia8Path.Length() > 0) {
            gSystem->Load(pythia8Path + "/lib/libpythia8");
        }
    }
...
void LoadPythiaEvent()
{
    ...
    // Generate event
    if(!pythia->next()) {
        cout << "Warning: Event generation failed!" << endl;
        return;
    }
    
    // Collect final state particles
    vector<PseudoJet> particles;
    vector<int> particleIndices;  // Store original Pythia indices
    
    // Particle selection cuts
    const double ptMin = 0.15;  // Minimum pT in GeV/c (typical tracking threshold)
    const double ptMax = 100.0; // Maximum pT in GeV/c
    const double etaMin = -0.9; // Minimum eta (typical ALICE acceptance)
    const double etaMax = 0.9;  // Maximum eta (typical ALICE acceptance)
    ...
    for(int i = 0; i < pythia->event.size(); i++) {
        ...
        // Select final state charged particles with pT and eta cuts
        if(p.isFinal() && p.isCharged() && 
           pt > ptMin && pt < ptMax && 
           eta > etaMin && eta < etaMax) {
            // Store track
            iTrk++;
            if(iTrk >= 100) break;  // Limit number of tracks
            tracks[iTrk].SetXYZ(p.px(), p.py(), p.pz());
            tracks[iTrk].SetUniqueID(i);  // Store Pythia index
            
            // Add to FastJet input (for jet finding)
            PseudoJet pj(p.px(), p.py(), p.pz(), p.e());
            pj.set_user_index(iTrk);  // Link back to track index
            particles.push_back(pj);
            particleIndices.push_back(i);
        }
    }
    
    // Find leading particle (LP)
    double maxPt = 0;
    int lpIndex = -1;
    for(int it = 0; it <= iTrk; it++) {
        if(tracks[it].Pt() > maxPt) {
            maxPt = tracks[it].Pt();
            lpIndex = it;
        }
    }
    ...
    vector<PseudoJet> jets_fj = sorted_by_pt(cs.inclusive_jets(5.0));  // pT > 5 GeV
    
    // Store jets and their constituents
    iJet = -1;
    for(unsigned int ij = 0; ij < jets_fj.size() && ij < 20; ij++) {
        PseudoJet& jet = jets_fj[ij];
        
        iJet++;
        jets[iJet].SetXYZ(jet.px(), jet.py(), jet.pz());
        jets[iJet].SetUniqueID(ij);
        
        // Get jet constituents
        vector<PseudoJet> constituents = jet.constituents();
        for(unsigned int ic = 0; ic < constituents.size(); ic++) {
            int trackIdx = constituents[ic].user_index();
            if(trackIdx >= 0 && trackIdx <= iTrk) {
                int pythiaIdx = tracks[trackIdx].GetUniqueID();
                cons[iJet].push_back(pythiaIdx);
                allcons.insert(pythiaIdx);
            }
        }
    }
}
...
class EvNavHandler
{
    public:
        ...
        void Bck(){
            if(ievent > 0) {
                ievent--;
                // Note: Pythia doesn't support going backwards, so we'll just regenerate
                // In a real implementation, you might want to store events
                LoadPythiaEvent();
                FillDeltaPlots();
                DrawDeltaPlots();
                DrawArrow();
...
void make_gui()
{
    ...
    TGHorizontalFrame* hf = new TGHorizontalFrame(frmMain);
    {
        ...
        b->SetToolTipText("Go to previous event (regenerates).");
        hf->AddFrame(b);
